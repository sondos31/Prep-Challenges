...
const objLat = (obj) => {
     ...
     else{ let firstCharInFirstName=((obj.firstName).charAt(0)).toUpperCase(); let firstCharInLastName=((obj.lastName).charAt(0)).toUpperCase(); obj.firstName=firstCharInFirstName+((obj.firstName).substring(1)).toLowerCase(); obj.lastName=firstCharInLastName+((obj.lastName).substring(1)).toLowerCase();
        return `my name is ${obj.firstName} ${obj.lastName} I am ${obj.age} YO, and I love ${obj.hobby}.` ; } };
// -------------------------------------------------------------------------------------------------------
...
{ if (arr[i].firstName !== null ) 
    ...
    let techCopy=arr[i].tech; arr[i]={ fullName:f_Name, tech:techCopy, }; }
    else{ let f_Name=arr[i].firstName; let techCopy=arr[i].tech; arr[i]={ fullName:f_Name, tech:techCopy, }; }; }
    else{ let f_Name =arr[i].lastName; let techCopy=arr[i].tech; arr[i]={ fullName:f_Name, tech:techCopy, }; }; }
    else{ nI=i; arr[i]={}; } }; const index = arr.indexOf(arr[nI]); if (index > -1) { arr.splice(index, 1); };
    return arr; };
// -------------------------------------------------------------------------------------------------------
